...
- `homebrew/Brewfile` — taps, CLI/dev `brew` formulae, and font casks; update with `brew bundle dump --force --file=homebrew/Brewfile` (app casks live in `homebrew/casks.json` instead — see below)
- `homebrew/casks.json` — manifest of app casks (name, description, `default`, optional `trusted`); `bootstrap.sh` installs `default: true` entries silently and prompts per-app for the rest. Hand-edit the `default` flags to change what installs without asking
...
- `.stow-local-ignore` — files Stow skips (README, bootstrap.sh, .git, config-plan.md)
